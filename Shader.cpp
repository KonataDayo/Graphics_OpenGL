...
unsigned int Shader::CompileShader(unsigned int type, const std::string& source) // unsigned int��ͬ��GLuint
{
    ...
    if (result == GL_FALSE)
    {
        ...
        char* message = (char*)alloca(length * sizeof(char)); // won't allocate on heap, instead still allocates on the stack
