...
void test::TestCube3D::OnUpdate(GLFWwindow* window, float deltaTime)
{
...
void test::TestCube3D::OnRender()
{
	...
	for (unsigned i = 0; i < 10 ; i++)
    {
        ...
        float x = 0;
		float y = 0;
        if (m_CubePositions[i].x >= 1920) 
			x = (float)glfwGetTime() * m_MoveSpeed * i / 1000;
		else if (m_CubePositions[i].x <= -1920)
            x = -(float)glfwGetTime() * m_MoveSpeed * i / 1000;
        if (m_CubePositions[i].y >= 960)
			y = + (float)glfwGetTime() * (float)glfwGetTime() * m_MoveSpeed * i / 1000;
        else if (m_CubePositions[i].y <= 960)
            y = -(float)glfwGetTime() * (float)glfwGetTime() * m_MoveSpeed * i / 1000;

		glm::vec3 trans(x, y, m_CubePositions[i].z);
...
void test::TestCube3D::OnImGuiRender()
{
    ...
    ImGui::SliderFloat("MoveSpeed", &m_MoveSpeed, -1.f, 1.f);
}
